...
public class WlanAvailableNetwork extends Structure {
  ...
  /**
   * Contains the profile name associated with the network. If the network does not have a profile,
   * this member will be empty. If multiple profiles are associated with the network, there will be
   * multiple entries with the same SSID in the visible network list. Profile names are
   ...
   *
   * Windows XP with SP3 and Wireless LAN API for Windows XP with SP2: uNumberOfBssids is at most 1,
   * regardless of the number of access points broadcasting the SSID.
   */
   ...
   * is WLAN_MAX_PHY_TYPE_NUMBER, which has a value of 8. If more than WLAN_MAX_PHY_TYPE_NUMBER PHY
   * types are supported, bMorePhyTypes must be set to TRUE.
   */
  public int uNumberOfPhyTypes;

  /**
   * Contains an array of DOT11_PHY_TYPE values that represent the PHY types supported by the
   * available networks. When uNumberOfPhyTypes is greater than WLAN_MAX_PHY_TYPE_NUMBER, this array
   * contains only the first WLAN_MAX_PHY_TYPE_NUMBER PHY types. dot11_phy_type_unknown		Specifies
   * an unknown or uninitialized PHY type. dot11_phy_type_any			Specifies any PHY type.
   * dot11_phy_type_fhss			Specifies a frequency-hoping spread-spectrum (FHSS) PHY. Bluetooth
   * devices can use FHSS or an adaption of FHSS. dot11_phy_tyep_dsss			Specifies a direct sequence
   * spread spectrum (DSSS) PHY. dot11_phy_type_irbaseband	Specifies an infrared (IR) baseband PHY.
   * dot11_phy_type_ofdm			Specifies an orthogonal frequency division multiplexing (OFDM) PHY.
   * 802.11a devices can use OFDM. dot11_phy_type_hrdsss		Specifies a high-rate DSSS (HRDSSS) PHY.
   * dot11_phy_type_erp			Specifies an extended rate PHY (ERP). 802.11g devices can use ERP.
   * dot11_phy_type_ht			Specifies an 802.11n PHY type. dot11_phy_type_vht			Specifies the 802.11ac
   * PHY type. This is the very high troughput PHY type specified in IEEE 802.11ac.
   *
   ...
   * are developed by an independent hardware vendor (IHV). dot11_phy_type_IHV_end		Specifies the
   * end of the range that is used to define PHY types that are developed by an independent hardware
   * vendor (IHV).
   */
   ...
   *
   * When this member is set to TRUE an application must call WlanGetNetworkBssList to get the
   * complete list of PHY types. The returned WLAN_BSS_LIST structure has an array of WLAN_BSS_ENTRY
   * structures. The uPhyId member of the WLAN_BSS_ENTRY structure contains the PHY type for an
   * entry.
   */
   ...
   * signal strength of -100 dbm. A value of 100 implies an actual RSSI signal strength of -50 dbm.
  ...
  /**
   * Indicates whether security is enabled on the network. A value of TRUE indicates that security
   * is enabled, otherwise it is not.
   */
  public boolean bSecurityEnabled;

  /**
   * A DOT11_AUTH_ALGORITHM value that indicates the default authentication algorithm used to join
  ...
  /**
   * Contains various flags for the network: WLAN_AVAILABLE_NETWORK_CONNECTED		Tis network is
   * currently connected. WLAN_AVAILABLE_NETWORK_HAS_PROFILE		There is a profile for this network.
   */
  public int dwFlags;
