...
public class WifiManager {
          ...
          case 8: //wlan_notification_source_acm
            ...
            switch (pNotifData.NotificationCode) {
              case 10: //wlan_notification_acm_connection_complete
                ...
                pConnNotifData = new WlanConnectionNotificationData.ByReference(
                    pNotifData.pData.getValue());
  ...
  private WlanInterfaceInfoList.ByReference enumInterfaces() {
    ...
    WlanInterfaceInfoList.ByReference pInterfaceInfoList = new WlanInterfaceInfoList.ByReference(
        p.getValue());
  ...
  private List<ScanResult> getVisibleNetworkList() {
    ...
    for (int i = 0; i < pAvailableNetworkList.dwNumberOfItems; i++) {
      ...
      String ssid = new String(Arrays
          .copyOfRange(pAvailableNetworkList.Network[i].dot11Ssid.ucSSID, 0, ssidArrayLength));
      String bssid = Integer.toString(pAvailableNetworkList.Network[i].uNumberOfBssids);
      String capabilities =
          Integer.toString(pAvailableNetworkList.Network[i].dot11DefaultAuthAlgorithm)
              + ", " + Integer
              .toString(pAvailableNetworkList.Network[i].dot11DefaultCipherAlgorithm);
  ...
  private List<String> getProfileList() {
    ...
    WlanProfileInfoList.ByReference pProfileList = new WlanProfileInfoList.ByReference(
        p.getValue());
  ...
  public String connectedSsid() {
    ...
    return new String(Arrays
        .copyOfRange(pWlanConnectionAttributes.wlanAssociationAttributes.dot11Ssid.ucSSID, 0,
            ssidArrayLength));
  }
  ...
  public String waitForDisconnect(long timeout, TimeUnit unit) throws InterruptedException {
    ...
    while ((!notificationCallback.isDisconnected()) && (System.currentTimeMillis()
        < startTime + timeToWaitInMs)) {
      Thread.sleep(100);
    }
  ...
  public String waitForConnect(String ssid, long timeout, TimeUnit unit)
    ...
    while ((!notificationCallback.isConnected()) && (System.currentTimeMillis()
        < startTime + timeToWaitInMs)) {
      Thread.sleep(100);
    }
